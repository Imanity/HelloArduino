	...
	public class XFileConverter {
		...
		private void Parser(string s) {
			if (Regex.IsMatch(s, "Material ")) {
				s = Regex.Replace(s, "Material ", "");	// 消除干扰 
		...
		private void CreateFolder() {
			...
			buf.Replace("/", "\\");
            System.IO.Directory.CreateDirectory(buf + "Materials");	// 首先生成Material的文件夹 
		}
		...
		private void EntrySubMeshForMesh(Mesh mesh) {
			...
			mesh.subMeshCount = matList.MaterialCount;	
            // 在这里设定子网格的数目
